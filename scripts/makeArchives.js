 ...
 * @param {String} [selectedTemplate] - optional name of a template. If specified
 * this is the only template that is built
...
async function buildTemplates(selectedTemplate) {
  ...
  for (const file of files) {
    ...
    if (fileName === 'package.json') {
      ...
      fs.writeFileSync(file, JSON.stringify(pkgJson), 'utf8', (err) => {
        if (err) {
          console.log(err);
        }
      });
