/* Generated using ergoc version 0.7.3 */
...
function main(context) {
  ...
  var res1 = null;
  if (either(cast(['org.accordproject.helloworld.MyRequest'], vX$match0))) {
    var vX$case0 = null;
    vX$case0 = toLeft(cast(['org.accordproject.helloworld.MyRequest'], vX$match0));
    res1 = { 'left': { '$main': vX$case0 } };
  } else {
    var vX$case1 = null;
    vX$case1 = toRight(cast(['org.accordproject.helloworld.MyRequest'], vX$match0));
    res1 = { 'right': null };
  }
  var res2 = null;
  if (either(res1)) {
    ...
    var vlstate = vstate;
    var vlemit = vemit;
    res2 = { left: concat(concat({ response: brand(['org.accordproject.helloworld.MyResponse'], { 'output': (`Hello ${  deref(unbrand(vcontract), "name")  } ${  deref(unbrand(vrequest), "input")}`) }) }, { 'state': vlstate }), { 'emit': vlemit }) };
  } else {
    var vX$1 = null;
    vX$1 = toRight(res1);
...
function helloworld(context) {
  ...
  var vlstate = vstate;
  var vlemit = vemit;
  return { 'left': concat(concat({ response: brand(['org.accordproject.helloworld.MyResponse'], { output: (`Hello ${  deref(unbrand(vcontract), "name")  } ${  deref(unbrand(vrequest), "input")}`) }) }, { 'state': vlstate }), { emit: vlemit }) };
}
...
t2 = typeof v2;
  if (t1 == 'object' && v1 !== null) {
    ...
    if (t1 != t2)
    {return t1 < t2 ? -1 : +1;}
  var a1 = {}.toString.apply(v1), 
a2 = {}.toString.apply(v2);
  if (a1 != a2)
    {return a1 < a2 ? -1 : +1;}
  if (a1 == '[object Array]') {
...
function compareOfMultipleCriterias(scl) {
  return function (a, b) {
    ...
    for (var i = 0; i < scl.length; i++) {
      var sc = scl[i];
      if ('asc' in sc) { current_compare = compare(deref(a, sc.asc), deref(b, sc.asc)); } else if ('desc' in sc) { current_compare = -(compare(deref(a, sc.asc), deref(b, sc.asc))); }

      if (current_compare == -1) { return -1; } else if (current_compare == 1) { return 1; }
    }
...
const data = { $class: 'org.accordproject.helloworld.HelloWorldClause', clauseId: 'c20434d0-973b-4102-8579-a706d2694949', 'name': 'Fred Blogs' };

const now = moment.parseZone();

init({ contract: data, emit: [], now, request: null });
